...
wss.on("connection", function(ws) {
	...
	if (Object.entries(games).length === 0 && games.constructor === Object) {
		...
		for (var game in games) {
			...
			if (games[game].waiting_for_players) {
				p1.x=Math.floor((Math.random() * 500) + 600);
				games[game].p2 = p1;
	...
	*/
		...
		if (p1.ySpeed === 0) {
		p1.x = Math.max(p1.x + p1.xSpeed * 2*4/30,-10);
		p1.x=Math.min(p1.x,3000);
	  } else {
		p1.x = Math.max(p1.x + p1.xSpeed * Math.sqrt(50)*4/150,-10);
		p1.x=Math.min(p1.x,3000);
	  }
	
	
	  if (p1.xSpeed === 0) {
		p1.y = Math.max(p1.y + p1.ySpeed * 2 *4/30,-10);
		p1.y=Math.min(p1.y,3000);
	  } else {
		p1.y =Math.max( p1.y + p1.ySpeed * Math.sqrt(50)*4/150,-10);
		p1.y=Math.min(p1.y,3000);
	  }
		...
		p1.reload = Math.max(p1.reload - 4, 0);
		games[game_id].bullets.forEach(function(element, index, object) {
			...
			if(element.playerId!=p1.id&&((Math.sqrt((buly-circ1Y)*(buly-circ1Y)+(bulx-circ1X)*(bulx-circ1X))<=46)||(Math.sqrt((buly-circ2Y)*(buly-circ2Y)+(bulx-circ2X)*(bulx-circ2X))<=47))){
				...
				*/
				games[game_id].p1_websocket.send(JSON.stringify(["dead", {dead:true, respawn: 200, id: p1.id, reload: p1.reload}]));
				games[game_id].p2_websocket.send(JSON.stringify(["dead", {dead:true, respawn: 200, id: p1.id, reload: p1.reload}]));
			}else{
